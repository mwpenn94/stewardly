/**
 * pastoral mission specialization (within Missional) — scaffold
 *
 ...
 * discipline. This is the scaffold for the pastoral mission; it accepts
 * intents on the same Intent contract and currently routes them
 * through the substrate's classifier-and-acknowledge default.
 *
 * To carry the pastoral mission to parity with wealth, add domain-specific
...
export const pastoralMissionHandler: EngineHandler = async (ctx, intent): Promise<IntentResult> => {
  ctx.log("info", "missional.pastoral.received", { kind: intent.kind });

  if (!intent.kind.startsWith("missional.pastoral.")) {
    return {
      ok: false,
      error: { code: "UNKNOWN_INTENT", message: `pastoral mission does not handle "${intent.kind}"` },
      invoked: [],
      cost: emptyCost(),
