...
// NOTE: original stewardly-ai imported its aiToolsRegistry from server/services.
// In Stewardly v3 the AI tools registry is being re-derived from the foundation's
// agentTools.ts (Phase 4 "clearly appropriate" decision). Until that integration
// is wired, we provide thin local stubs so the seed file remains valid.
type ToolDefinition = { toolName: string };
async function registerTool(_: ToolDefinition | unknown): Promise<void> { /* TODO: wire to aiToolsRegistry */ }
async function getToolByName(_: string): Promise<ToolDefinition | null> { return null; }
export { registerTool, getToolByName };
