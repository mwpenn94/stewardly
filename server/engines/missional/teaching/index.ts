/**
 * teaching mission specialization (within Missional) — scaffold
 *
 ...
 * discipline. This is the scaffold for the teaching mission; it accepts
 * intents on the same Intent contract and currently routes them
 * through the substrate's classifier-and-acknowledge default.
 *
 * To carry the teaching mission to parity with wealth, add domain-specific
...
export const teachingMissionHandler: EngineHandler = async (ctx, intent): Promise<IntentResult> => {
  ctx.log("info", "missional.teaching.received", { kind: intent.kind });

  if (!intent.kind.startsWith("missional.teaching.")) {
    return {
      ok: false,
      error: { code: "UNKNOWN_INTENT", message: `teaching mission does not handle "${intent.kind}"` },
      invoked: [],
      cost: emptyCost(),
