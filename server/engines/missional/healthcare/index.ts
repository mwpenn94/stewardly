/**
 * healthcare mission specialization (within Missional) — scaffold
 *
 ...
 * discipline. This is the scaffold for the healthcare mission; it accepts
 * intents on the same Intent contract and currently routes them
 * through the substrate's classifier-and-acknowledge default.
 *
 * To carry the healthcare mission to parity with wealth, add domain-specific
...
export const healthcareMissionHandler: EngineHandler = async (ctx, intent): Promise<IntentResult> => {
  ctx.log("info", "missional.healthcare.received", { kind: intent.kind });

  if (!intent.kind.startsWith("missional.healthcare.")) {
    return {
      ok: false,
      error: { code: "UNKNOWN_INTENT", message: `healthcare mission does not handle "${intent.kind}"` },
      invoked: [],
      cost: emptyCost(),
