/**
 * coaching mission specialization (within Missional) — scaffold
 *
 ...
 * discipline. This is the scaffold for the coaching mission; it accepts
 * intents on the same Intent contract and currently routes them
 * through the substrate's classifier-and-acknowledge default.
 *
 * To carry the coaching mission to parity with wealth, add domain-specific
...
export const coachingMissionHandler: EngineHandler = async (ctx, intent): Promise<IntentResult> => {
  ctx.log("info", "missional.coaching.received", { kind: intent.kind });

  if (!intent.kind.startsWith("missional.coaching.")) {
    return {
      ok: false,
      error: { code: "UNKNOWN_INTENT", message: `coaching mission does not handle "${intent.kind}"` },
      invoked: [],
      cost: emptyCost(),
