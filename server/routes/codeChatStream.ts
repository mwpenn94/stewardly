...
// R14.19.step1: agent executor alias route. Both endpoints share the same
// handler so frontends can migrate at their own pace before step3 removes the
// old name. Define the handler once, mount it twice.
const codeChatStreamHandler = async (req: any, res: any) => {
...
// R14.19.step1: register the same handler under both names. The agentExecutor
// alias is the new canonical name; codechat is kept until step3 completes the
// frontend migration.
codeChatStreamRouter.post("/api/codechat/stream", codeChatStreamHandler);
codeChatStreamRouter.post("/api/agent-executor/stream", codeChatStreamHandler);
