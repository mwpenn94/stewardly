                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-1.5 mt-1">
                    {(rolesQ.data ?? []).slice(0, 9).map((r: any) => (
                      <button type="button" key={r.key} onClick={() => setRole(r.key)}
                        className={`text-[10px] py-1.5 rounded border transition-all ${role === r.key ? "bg-primary/10 border-primary/30 text-primary" : "bg-card/40 border-border/50 text-muted-foreground"}`}>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="space-y-1">
                            {[
                              ...
                              { label: "Placed Cases", value: funnel.placed, color: "bg-primary/20 border-primary/30" },
                            ].map((step, i) => (
                ...
                <CardContent>
                  <div className="space-y-1">
                    {bracketsQ.data.map((b: any, i: number) => {
                      ...
                      return (
                        <div key={i} className={`flex items-center justify-between px-2 py-1.5 rounded text-xs transition-colors ${isActive ? "bg-primary/12 border border-primary/30" : "bg-card/30"}`}>
