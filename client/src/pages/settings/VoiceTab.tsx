...
const LS_TTS_ENGINE = "tts-engine"; // "edge" | "device"
...
export default function VoiceTab() {
  ...
  const [ttsEngine, setTtsEngineState] = useState<"edge" | "device">(() => {
    const v = safeGetItem(LS_TTS_ENGINE);
    return v === "device" ? "device" : "edge";
  });
  // Discover device voices for the device-TTS preview
  ...
  useEffect(() => { safeSetItem(LS_TTS_ENGINE, ttsEngine); }, [ttsEngine]);
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm flex items-center gap-2">
            <Headphones className="w-4 h-4 text-primary" /> Speech Engine
          </CardTitle>
          <CardDescription className="text-xs">
            Choose between Microsoft Edge TTS (premium neural voices, requires network) or your device's built-in speech voice (offline-capable, voice quality depends on your OS / browser). No third-party AI voice services are used.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <button
              type="button"
              onClick={() => setTtsEngineState("edge")}
              className={`text-left p-3 rounded-lg border transition-all ${
                ttsEngine === "edge"
                  ? "border-accent bg-primary/10 ring-1 ring-accent/30"
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="font-medium text-sm">Microsoft Edge TTS</span>
                {ttsEngine === "edge" && <Badge variant="default" className="text-[9px] ml-auto">Active</Badge>}
              </div>
              <p className="text-xs text-muted-foreground">
                Premium neural voices. 26+ accents and personas. Server-streamed MP3.
              </p>
            </button>
            <button
              type="button"
              onClick={() => setTtsEngineState("device")}
              className={`text-left p-3 rounded-lg border transition-all ${
                ttsEngine === "device"
                  ? "border-accent bg-primary/10 ring-1 ring-accent/30"
                  : "border-border bg-secondary/30 hover:border-primary/40"
              }`}
              data-testid="tts-engine-device"
            >
              <div className="flex items-center gap-2 mb-1">
                <Mic className="w-4 h-4 text-primary" />
                <span className="font-medium text-sm">Device Voice</span>
                {ttsEngine === "device" && <Badge variant="default" className="text-[9px] ml-auto">Active</Badge>}
              </div>
              <p className="text-xs text-muted-foreground">
                Uses your operating system's built-in speech voice ({deviceVoices.length} installed). Works offline.
              </p>
            </button>
          </div>
        </CardContent>
