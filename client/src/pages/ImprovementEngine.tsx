      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <section className="mb-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
            {/* R14.25.e: overflow-x-auto + whitespace-nowrap so direction tabs scroll instead of overflow on narrow phones */}
            <div className="flex gap-2 overflow-x-auto scrollbar-hide w-full sm:w-auto pb-1">
              {DIRECTIONS.map(dir => (
                <button type="button"
                  key={dir.key}
                  onClick={() => setSelectedDirection(dir.key)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm transition-colors whitespace-nowrap shrink-0 ${
                    selectedDirection === dir.key
                      ? `bg-primary/20 ${dir.color} font-semibold`
              ...
              {DIRECTIONS.map(dir => {
                ...
                return (
                  <div key={dir.key} className={`p-4 rounded-xl border ${
                    selectedDirection === dir.key ? "border-primary/40 bg-primary/5" : "border-border/30 bg-card"
                  }`}>
