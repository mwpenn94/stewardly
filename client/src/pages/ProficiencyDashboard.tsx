...
const LAYER_CONFIG: Record<string, { icon: React.ReactNode; color: string; bgColor: string; borderColor: string }> = {
  ...
  client: { icon: <Brain className="w-4 h-4" />, color: "text-primary", bgColor: "bg-primary/10", borderColor: "border-primary/30" },
};
...
const PROFICIENCY_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  ...
  familiar: { bg: "bg-primary/20", text: "text-primary/80", label: "Familiar" },
  ...
  return (
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        {/* Guest CTA Banner */}
        {isGuest && (
          <Card className="border-primary/20 bg-gradient-to-r from-accent/5 to-violet-500/5">
                  ...
                  <div className="space-y-2">
                    {ins.nextSteps.map((step, i) => (
                      <button type="button"
                        ...
                        className="w-full flex items-start gap-3 p-3 rounded-lg bg-card/50 border border-border/50 hover:border-primary/30 hover:bg-card transition-all text-left group"
                      >
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            {isActive && (
                              <Badge variant="outline" className="ml-2 text-[10px] px-1 py-0 border-primary/30 text-primary">
                                Active
                              </Badge>
                            )}
                          </div>
