  ...
  return (
    <Shell title="Comparables">
      ...
      <main
        id="comparables-main"
        tabIndex={-1}
        // R14.25.f: explicit bottom padding so the last card never sits
        // under the floating + FAB / Voice ORB on mobile, even when this
        // page is rendered inside an embed where AppLayout’s 9rem floor
        // does not apply.
        className="mx-auto max-w-6xl space-y-6 p-6 pb-32 md:pb-6"
      >
                            ...
                            <div className="flex flex-wrap gap-1">
                              {row.leaders.map((l) => (
                                <button type="button"
                                  ...
                                  className="rounded border border-border/60 px-1.5 py-0.5 text-[11px] text-muted-foreground hover:border-accent hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                                >
                                  {l.app.name}
                  ...
                  >
                    ...
                    <span className="flex-1 text-sm font-medium">
                      ...
                      {row.isStewardly && (
                        <Badge
                          variant="outline"
                          className="ml-2 border-primary/40 bg-primary/10 text-[10px] text-primary"
                        >
                          YOU
                        </Badge>
                      )}
