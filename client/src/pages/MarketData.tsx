  ...
  return (
    ...
    <div className="min-h-screen">
      <header className="border-b border-border px-4 py-3 flex items-center gap-3 relative overflow-hidden">
        ...
        <h2 className="text-lg font-semibold relative">Market Data</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {quotesQuery.isLoading ? (
              ...
              (Array.isArray(quotesQuery.data) ? quotesQuery.data : []).map((q: any, idx: number) => (
                <Card
                  key={q.symbol || idx}
                  className={`cursor-pointer transition-all hover:border-primary/40 ${selectedSymbol === q.symbol ? "border-primary/60 bg-primary/5" : ""}`}
