...
export default function OnboardingChecklist({
  ...
  return (
    <div className={`rounded-xl border ${isComplete ? "border-emerald-500/30 bg-emerald-500/5" : "border-primary/20 bg-primary/5"} backdrop-blur-sm overflow-hidden transition-all`}>
      {/* Header */}
      <div
        className="flex items-center gap-3 p-4 cursor-pointer select-none"
        onClick={() => setExpanded(!expanded)}
      >
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isComplete ? "bg-emerald-500/20" : "bg-primary/15"}`}>
