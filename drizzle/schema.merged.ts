// AUTO-GENERATED — Stewardly v3 merged schema for drizzle-kit generation.
// Re-run scripts/03_build_merged_schema.py to regenerate.
// Source files: drizzle/schema.ts (foundation), migrations/inputs/additive-tables.ts.

// 1) Re-export foundation tables as-is.
export * from "./schema";

// 2) Imports for additive tables (mirrors stewardly-ai/drizzle/schema.ts).
import {
  int, mysqlEnum, mysqlTable, text, timestamp, varchar, json, float,
  boolean as mysqlBoolean, bigint, decimal, date, index, uniqueIndex, double,
} from "drizzle-orm/mysql-core";
import { nanoid } from "nanoid";

// 3) Additive tables ported from stewardly-ai (414 tables, additive-only).
...
// 4) Overlap-table augmentations — additive columns from stewardly-ai.
//    Same physical MySQL table, so drizzle-kit emits ALTER TABLE ... ADD COLUMN.
export const usersStewardlyExt = mysqlTable("users", {
  // Foundation primary key (kept identical so drizzle-kit recognizes the table)
  id: int("id").autoincrement().primaryKey(),
...
export const tasksStewardlyExt = mysqlTable("tasks", {
  // Foundation primary key (kept identical so drizzle-kit recognizes the table)
  id: int("id").autoincrement().primaryKey(),
  assignedTo: int("assigned_to"),
  clientId: int("client_id"),
...
export const userPreferencesStewardlyExt = mysqlTable("user_preferences", {
  // Foundation primary key (kept identical so drizzle-kit recognizes the table)
  id: int("id").autoincrement().primaryKey(),
